# def income_collector(number_of_incomes, income_list_value):
...
# def expense_collector(income_list_input, expense_list_length):
#   j = 0
...
# incomes = income_collector(number_of_incomes, income_list_value)
# expenses_for_each_income = expense_collector(incomes)
# budget_splitter(incomes, expenses_for_each_income)


# if __name__ == "__main__":
#     number_of_incomes = int(input("Enter the number of incomes: "))
#     income_list_value = input("Enter the income list values (comma-separated): ")

#     incomes = income_collector(number_of_incomes, income_list_value)
#     expenses_for_each_income = expense_collector(incomes, len(incomes))
#     budget_splitter(incomes, expenses_for_each_income)
...
def expense_collector(income_list_input, expense_list_length):
    ...
    for income in income_list_input:
        ...
        # Collects expenses
        i = 0
        while i < int(expense_list_length):
            expense_list_value = input(f"Expense # {i + 1} cost in dollars: > ")
            expenses_for_this_income += int(expense_list_value)
            i += 1
        expense_list[j] = expenses_for_this_income
